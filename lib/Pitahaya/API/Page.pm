...
sub children {
  ...
  my $tx =
    $self->api->ua->get(
    $self->api->url . "/admin/" . $self->site->name . "/page/tree/children/" . $self->id,
    { Accept => "application/json" } );
  ...
  if ( $tx->success ) {
    ...
    my @ret;
    for my $child ( @{$ref} ) {
      push @ret, Pitahaya::API::Page->new({site => $self->site, api => $self->api, id => $child->{id}});
    }
...
sub add_to_children {
  ...
  if ( $tx->success ) {
    my $ref = $tx->res->json;
    return Pitahaya::API::Page->new(site => $self->site, api => $self->api, id => $ref->{id});
  }
}
